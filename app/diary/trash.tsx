...
const TrashPage = () => {
  ...
  const fetchDeletedDiaries = async () => {
    try {
      ...
      CustomAlertManager.error(t.locale.startsWith('en') ? 'Failed to load trash diary.' : '휴지통 일기를 불러오는데 실패했습니다.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleRestore = async (id: number, title: string | null) => {
    const confirmed = await CustomAlertManager.confirm(
      ...
      t.locale.startsWith('en') ? `Do you want to restore the diary "${title || 'Untitled'}"?` : `"${title || '제목 없음'}" 일기를 복원하시겠습니까?`
    );
    
    if (confirmed) {
      try {
        await DiaryService.restoreDiary(id);
        CustomAlertManager.success(t.locale.startsWith('en') ? 'Diary has been restored.' : '일기가 복원되었습니다.');
        fetchDeletedDiaries();
      } catch (error) {
        console.error('일기 복원 실패:', error);
        CustomAlertManager.error(t.locale.startsWith('en') ? 'Failed to restore diary.' : '일기 복원에 실패했습니다.');
      }
    }
  };
  ...
  const handlePermanentDelete = async (id: number, title: string | null) => {
    const confirmed = await CustomAlertManager.confirm(
      ...
      t.locale.startsWith('en') ? `Do you want to permanently delete the diary "${title || 'Untitled'}"?\n\nThis action cannot be undone.` : `"${title || '제목 없음'}" 일기를 영구적으로 삭제하시겠습니까?\n\n이 작업은 되돌릴 수 없습니다.`
    );
    
    if (confirmed) {
      try {
        await DiaryService.permanentDeleteDiary(id);
        CustomAlertManager.success(t.locale.startsWith('en') ? 'Diary has been permanently deleted.' : '일기가 영구적으로 삭제되었습니다.');
        fetchDeletedDiaries();
      } catch (error) {
        console.error('일기 영구 삭제 실패:', error);
        CustomAlertManager.error(t.locale.startsWith('en') ? 'Failed to delete diary.' : '일기 삭제에 실패했습니다.');
      }
    }
  };
      ...
      >
        {loading ? (
          ...
          <View style={{
            ...
            }}>{t.locale.startsWith('en') ? 'Trash is empty' : '휴지통이 비어있습니다'}</Text>
            <Text style={{
              marginTop: 8,
              fontSize: 14,
              color: themeColors.textSecondary,
            }}>{t.locale.startsWith('en') ? 'No deleted diaries' : '삭제된 일기가 없습니다'}</Text>
          </View>
        ) : (
              ...
              return (
                <View key={diary.id} style={{ marginBottom: 16 }}>
                  ...
                  <View style={{
                    ...
                    numberOfLines={1}
                    >
{diary.deleted_at ? formatDateTimeString(diary.deleted_at, t.locale.startsWith('en') ? 'en' : 'ko') : ''}
                    </Text>
