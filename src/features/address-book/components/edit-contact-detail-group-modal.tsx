...
const EditContactDetailGroupModal = ({
  ...
  const handleEditContactDetailGroup = async () => {
    if (!groupName.trim()) {
      CustomAlertManager.error(isEnglish ? 'Please enter a list name.' : '목록 이름을 입력해주세요.');
      return;
    }
      ...
      <View style={{ paddingHorizontal: 24, paddingVertical: 16 }}>
        ...
        <View style={{ marginBottom: 24 }}>
          ...
          <TextInput
            ...
            placeholder={group ? group.title : (isEnglish ? 'Please enter a list name' : '목록 이름을 입력해주세요')}
